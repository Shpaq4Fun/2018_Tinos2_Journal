...
Faculty of Geoengineering, Mining and Geology, Wroc{\l}aw University of Science and Technology, Wroc{\l}aw, Poland}\\
\IEEEauthorblockA{\IEEEauthorrefmark{4}Faculty of Pure and Applied Mathematics, Hugo Steinhaus Centre, Wroc{\l}aw University of Science and Technology, Wroc{\l}aw, Poland}}
...
%In principle, normalizaton is performed by the columns of matrix \textbf{W} %\cite{cichocki2009nonnegative}:

%\begin{equation}   \mathbf{W} \leftarrow \mathbf{WD_W},\end{equation}
...
%\end{equation}
 ...
 %in our case input matrix \textbf{V} is transposed, output matrices of NMF algorithm are %inverted in their functionality. Hence, in practice 
...
% In this article first we analyze the simulated signal. It  consists of cyclic and noncyclic impulses. The former ones have cyclic frequency equal to 40 Hz and carrier frequency around $2500-3500$ Hz. The latter impulses have a center frequency equal to 5000 Hz, but bandwidths which varies. The  MATLAB function \emph{gauspuls} with a center frequency $F_c = 3000$ Hz and uniformly distributed bandwidth on [0.2, 0.3] interval is used to reproduce the cyclic impulses. As a next step, a single impulse is replicated to obtain cyclic signal. Furthermore, in case of noncyclic impulse the same function is applied, however with different  centre frequency $F_c = 5000$ Hz and uniformly distributed bandwidth on [0, 1] interval. Positions of the noncyclic impulses are uniformly distributed on the time axis. Finally, the Gaussian noise is added to the signal. The model of the simulated signal is presented in Fig. \ref{fig:schemat sygnal}.
% Till now, most of diagnostic  procedures  assumed  that  informative signal (SOI - signal  of  Interest) is hidden in Gaussian noise. In such case, prefiltering procedures based on such modification of filter coefficients that provides the most impulsive signal after prefiltering. The most popular parameter used for such denoising was kurtosis [literatura]. Unfortunately, in case of the presence of other impulsive (non-Gaussian) contribution but not related to damage, classic methods cannot be used. Presence of non-Gaussian noise makes diagnostics very complicated and there is no simple solution. Using Hidden Markov Regime Switching Model was proposed in \cite{wylomanskaimpulsive} however, it is pretty complicated technique and requires pre-identification of impulsive noise characteristics that might be really difficult in practice.

% \begin{figure}[ht!]
%     \centering
%     \includegraphics[width = 0.48\textwidth]{figs/sym_signal_components.png.png}
%      \caption{ The schematic model of the signal,by the courtesy of the Authors of \cite{wylomanskaimpulsive}}
%     \label{fig:schemat sygnal}
% \end{figure}
